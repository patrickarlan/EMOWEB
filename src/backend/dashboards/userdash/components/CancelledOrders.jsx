...
export default function CancelledOrders() {
  ...
  const getStatusColor = (status) => {
    return '#f87171'; // Red for cancelled
  };
    ...
    return (
      <div className="orders-container">
        <div className="orders-error">
          <p>Error loading cancelled orders: {error}</p>
          <button onClick={fetchCancelledOrders} className="retry-button">
            Try Again
          </button>
        </div>
      </div>
  ...
  return (
    <div className="orders-container">
      <div className="orders-header">
        ...
        <p className="orders-subtitle">Your order cancellation history</p>
      </div>

      <div className="orders-list">
        {orders.map((order) => (
          <div key={order.id} className="order-card cancelled-order-card">
            ...
            {order.cancellation_reason && (
              <div className="cancellation-reason-box">
                <span className="reason-label">Cancellation Reason:</span>
                <span className="reason-text">{order.cancellation_reason}</span>
              </div>
            )}

            {order.cancelled_at && (
              <div className="cancellation-date">
                <span className="cancelled-label">Cancelled on:</span>
                <span className="cancelled-date-text">{formatDate(order.cancelled_at)}</span>
              </div>
            )}
